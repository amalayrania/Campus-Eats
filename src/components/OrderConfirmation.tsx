...
export default function OrderConfirmation({
  ...
  // Guard against unusually long create calls so users can recover
  useEffect(() => {
    if (!isCreating || error || createdOrder) {
      return;
    }

    const timeoutId = window.setTimeout(() => {
      setError('Order is taking longer than expected. Please try again.');
      setIsCreating(false);
    }, 12000);

    return () => window.clearTimeout(timeoutId);
  }, [createdOrder, error, isCreating]);
  ...
  if (isCreating) {
    return (
      <div className="h-full bg-[#F8F9FA] flex flex-col items-center justify-center px-6">
        ...
        <button
          onClick={() => {
            setError('Order was canceled. Please try again.');
            setIsCreating(false);
          }}
          className="mt-6 px-4 py-2 text-[#2D6A4F] border border-[#2D6A4F] rounded-xl hover:bg-[#F0FDF4] transition-colors active:scale-95"
        >
          Cancel and go back
        </button>
      </div>
